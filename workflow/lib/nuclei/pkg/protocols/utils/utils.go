...
// CleanStructFieldJSONTag cleans taskstruct json tag field
...
func AddConfiguredClientCertToRequest(tlsConfig *tls.Config, options *types.Options) (*tls.Config, error) {
	// Build the TLS workflowstruct with the client certificate if it has been configured with the appropriate options.
