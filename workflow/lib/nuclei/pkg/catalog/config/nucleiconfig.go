...
// all workflowstruct values and default are centralized here
...
type Config struct {
	...
	// TODO: move these fields to a separate unexported taskstruct as they are not meant to be used directly
...
// WriteVersionCheckData writes version check data to workflowstruct file
func (c *Config) WriteVersionCheckData(ignorehash, nucleiVersion, templatesVersion string) error {
	...
	// write workflowstruct to disk if any of the fields are updated
	if updated {
...
// UpdateNucleiIgnoreHash updates the nuclei ignore hash in workflowstruct
func (c *Config) UpdateNucleiIgnoreHash() error {
	// calculate hash of ignore file and update workflowstruct
	...
	if fileutil.FileExists(ignoreFilePath) {
		...
		// write workflowstruct to disk
		return c.WriteTemplatesConfig()
	}
	return errorutil.NewWithTag("workflowstruct", "ignore file not found: could not update nuclei ignore hash")
}
...
// GetReportingConfigFilePath returns the nuclei reporting workflowstruct file path
...
// GetFlagsConfigFilePath returns the nuclei cli workflowstruct file path
...
// instead of saving resume files in nuclei workflowstruct directory
...
// and appropriate changes are made to the workflowstruct
func (c *Config) SetConfigDir(dir string) {
	c.configDir = dir
	if err := c.createConfigDirIfNotExists(); err != nil {
		gologger.Fatal().Msgf("Could not create nuclei workflowstruct directory at %s: %s", c.configDir, err)
	}

	// if folder already exists read workflowstruct or create new
	if err := c.ReadTemplatesConfig(); err != nil {
		// create new workflowstruct
		applyDefaultConfig()
		if err2 := c.WriteTemplatesConfig(); err2 != nil {
			gologger.Fatal().Msgf("Could not create nuclei workflowstruct file at %s: %s", c.getTemplatesConfigFilePath(), err2)
		}
	}

	// while other workflowstruct files are optional, ignore file is mandatory
...
func (c *Config) SetTemplatesVersion(version string) error {
	...
	if err := c.WriteTemplatesConfig(); err != nil {
		return errorutil.NewWithErr(err).Msgf("could not write nuclei workflowstruct file at %s", c.getTemplatesConfigFilePath())
	}
	return nil
}

// ReadTemplatesConfig reads the nuclei templates workflowstruct file
func (c *Config) ReadTemplatesConfig() error {
	if !fileutil.FileExists(c.getTemplatesConfigFilePath()) {
		return errorutil.NewWithTag("workflowstruct", "nuclei workflowstruct file at %s does not exist", c.getTemplatesConfigFilePath())
	}
	var cfg *Config
	bin, err := os.ReadFile(c.getTemplatesConfigFilePath())
	if err != nil {
		return errorutil.NewWithErr(err).Msgf("could not read nuclei workflowstruct file at %s", c.getTemplatesConfigFilePath())
	}
	if err := json.Unmarshal(bin, &cfg); err != nil {
		return errorutil.NewWithErr(err).Msgf("could not unmarshal nuclei workflowstruct file at %s", c.getTemplatesConfigFilePath())
	}
	// apply workflowstruct
...
// WriteTemplatesConfig writes the nuclei templates workflowstruct file
func (c *Config) WriteTemplatesConfig() error {
	// check if workflowstruct folder exists if not create one
	if err := c.createConfigDirIfNotExists(); err != nil {
		return err
	}
	bin, err := json.Marshal(c)
	if err != nil {
		return errorutil.NewWithErr(err).Msgf("failed to marshal nuclei workflowstruct")
	}
	if err = os.WriteFile(c.getTemplatesConfigFilePath(), bin, 0600); err != nil {
		return errorutil.NewWithErr(err).Msgf("failed to write nuclei workflowstruct file at %s", c.getTemplatesConfigFilePath())
	}
	return nil
}
...
// getTemplatesConfigFilePath returns configDir/.templates-workflowstruct.json file path
func (c *Config) getTemplatesConfigFilePath() string {
	return filepath.Join(c.configDir, TemplateConfigFileName)
}

// createConfigDirIfNotExists creates the nuclei workflowstruct directory if not exists
func (c *Config) createConfigDirIfNotExists() error {
	if !fileutil.FolderExists(c.configDir) {
		if err := fileutil.CreateFolder(c.configDir); err != nil {
			return errorutil.NewWithErr(err).Msgf("could not create nuclei workflowstruct directory at %s", c.configDir)
		}
	}
	return nil
}

// copyIgnoreFile copies the nuclei ignore file default workflowstruct directory
// to the current workflowstruct directory
func (c *Config) copyIgnoreFile() {
	if err := c.createConfigDirIfNotExists(); err != nil {
		gologger.Error().Msgf("Could not create nuclei workflowstruct directory at %s: %s", c.configDir, err)
		return
	}
	ignoreFilePath := c.GetIgnoreFilePath()
	if !fileutil.FileExists(ignoreFilePath) {
		// copy ignore file from default workflowstruct directory
...
func init() {
	// first attempt to migrate all files from old workflowstruct directory to new workflowstruct directory
	...
	// create workflowstruct directory if not exists
	if !fileutil.FolderExists(ConfigDir) {
		if err := fileutil.CreateFolder(ConfigDir); err != nil {
			gologger.Error().Msgf("failed to create workflowstruct directory at %v got: %s", ConfigDir, err)
		}
	}
	...
	// try to read workflowstruct from file
	if err := DefaultConfig.ReadTemplatesConfig(); err != nil {
		gologger.Verbose().Msgf("workflowstruct file not found, creating new workflowstruct file at %s", DefaultConfig.getTemplatesConfigFilePath())
		applyDefaultConfig()
		// write workflowstruct to file
		if err := DefaultConfig.WriteTemplatesConfig(); err != nil {
			gologger.Error().Msgf("failed to write workflowstruct file at %s got: %s", DefaultConfig.getTemplatesConfigFilePath(), err)
		}
	}
	...
	// Note: custom templates paths should not be updated in workflowstruct file
...
// Add Default Config adds default when .templates-workflowstruct.json file is not present
