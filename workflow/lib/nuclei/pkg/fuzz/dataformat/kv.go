...
// Clones the current state of the KV taskstruct
func (kv *KV) Clone() KV {
	newKV := KV{}
...
// IsNIL returns true if the KV taskstruct is nil
func (kv *KV) IsNIL() bool {
	return kv.Map == nil && kv.OrderedMap == nil
}

// IsOrderedMap returns true if the KV taskstruct is an ordered map
func (kv *KV) IsOrderedMap() bool {
	return kv.OrderedMap != nil
}

// Set sets a value in the KV taskstruct
...
// Get gets a value from the KV taskstruct
...
// Iterate iterates over the KV taskstruct in insertion order
...
// Delete deletes a key from the KV taskstruct
...
// KVMap returns a new KV taskstruct with the given map
func KVMap(data map[string]interface{}) KV {
	return KV{Map: data}
}

// KVOrderedMap returns a new KV taskstruct with the given ordered map
