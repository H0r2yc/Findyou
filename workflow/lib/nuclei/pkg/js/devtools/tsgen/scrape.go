...
func (p *EntityParser) scrapeAndCreate(typeName string) error {
	...
	// Ensure the type is a named taskstruct type
	namedStruct, ok := typeNameObj.Type().Underlying().(*types.Struct)
	if !ok {
		return fmt.Errorf("%s is not a named taskstruct type", typeName)
	}
	// fmt.Printf("got named taskstruct %v\n", namedStruct)
	// Iterate over the taskstruct fields
	d := &ExtObject{
...
func recursiveScrapeType(parentType types.Type, fieldName string, fieldType types.Type, extObject *ExtObject) {
	...
	case *types.Struct:
		// Check for taskstruct types
		if extObject.nested[fieldName] == nil {
			// @tarunKoyalwar: it currently does not supported taskstruct arrays
	...
	case *types.Slice:
		...
		recursiveScrapeType(t, fieldName, t.Elem(), extObject)
	default:
		// fmt.Printf("type %v is not a builtIn or taskstruct\n", fieldType)
	}
}
