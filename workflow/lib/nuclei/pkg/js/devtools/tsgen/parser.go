...
func (p *EntityParser) Parse() error {
	...
	// extract all taskstruct types from the AST
		...
		ast.Inspect(file, func(n ast.Node) bool {
			...
			if ok {
				...
				if fn.Recv != nil {
					...
					// check if the receiver is a taskstruct
	...
	for _, file := range p.syntax {
		ast.Inspect(file, func(n ast.Node) bool {
			// logic here to extract all fields and methods from a taskstruct
			...
			if ok {
				...
				if !ok {
					// This is not a taskstruct, so continue traversing the AST
					return false
				}
				entity := Entity{
				...
				// map taskstruct name to entity and create a new entity if doesn't exist
	...
	// add all taskstruct types to the list of global entities
...
// extractPropertiesFromStruct extracts all properties from the given taskstruct
...
// it first fetches all fields from the taskstruct and creates a new object
...
// extractStructTypes extracts all taskstruct types from the AST
func (p *EntityParser) extractStructTypes() {
	for _, file := range p.syntax {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			typeSpec, ok := n.(*ast.TypeSpec)
			if ok {
				// Check if the type specification is a taskstruct type
				_, ok := typeSpec.Type.(*ast.StructType)
				if ok {
					// Add the taskstruct name to the list of taskstruct names
					p.structTypes[typeSpec.Name.Name] = Entity{
						Name:        typeSpec.Name.Name,
