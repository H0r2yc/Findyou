 ...
 * @example
 * ```javascript
 * const mysqldb = require('nuclei/mysqldb');
 * const options = new mysqldb.MySQLOptions();
 * options.Host = 'acme.com';
 * options.Port = 3306;
 * const dsn = mysqldb.BuildDSN(options);
 * ```
 */
 ...
 * Internally client uses go-sql-driver/mysqldb driver.
 * @example
 * ```javascript
 * const mysqldb = require('nuclei/mysqldb');
 * const client = new mysqldb.MySQLClient;
 * ```
 */
export class MySQLClient {
    ...
    * ```javascript
    * const mysqldb = require('nuclei/mysqldb');
    * const isMySQL = mysqldb.IsMySQL('acme.com', 3306);
    * ```
    */
    ...
    * ```javascript
    * const mysqldb = require('nuclei/mysqldb');
    * const info = mysqldb.FingerprintMySQL('acme.com', 3306);
    * log(to_json(info));
    * ```
    */
    ...
    * we override mysqldb dialer with fastdialer so it respects network policy
    * If connection is successful, it returns true.
    * @example
    * ```javascript
    * const mysqldb = require('nuclei/mysqldb');
    ...
    * options.Host = 'acme.com';
    * options.Port = 3306;
    * const result = mysqldb.ExecuteQueryWithOpts(options, 'SELECT * FROM users');
    * log(to_json(result));
    * ```
    */
    ...
    * ```javascript
    * const mysqldb = require('nuclei/mysqldb');
    * const result = mysqldb.ExecuteQuery('acme.com', 3306, 'username', 'password', 'SELECT * FROM users');
    * log(to_json(result));
    * ```
    */
    ...
    * ```javascript
    * const mysqldb = require('nuclei/mysqldb');
    * const result = mysqldb.ExecuteQueryOnDB('acme.com', 3306, 'username', 'password', 'dbname', 'SELECT * FROM users');
    * log(to_json(result));
    * ```
    */
