...
func (r *Runner) RunEnumeration(TargetAndPocsName map[string][]string) error {
	...
	// workflowstruct.json - containing below info
...
// Walk traverses a taskstruct and executes a callback function on each value in the taskstruct.
// The interface{} passed to the function should be a pointer to a taskstruct or a taskstruct.
// WalkFunc is the callback function used for each value in the taskstruct. It is passed the
// reflect.Value and reflect.Type properties of the value in the taskstruct.
...
// expandEndVars looks for values in a taskstruct tagged with "yaml" and checks if they are prefixed with '$'.
