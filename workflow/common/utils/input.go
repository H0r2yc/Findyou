...
func GetInputType(input string) int {
	if IsIPv6(input) {
		return workflowstruct.TypeUnSupport
	} else if IsIPv4(input) {
		return workflowstruct.TypeIP
	} else if IsIPRange(input) {
		return workflowstruct.TypeIPRange
	} else if IsCIDR(input) {
		return workflowstruct.TypeCIDR
	} else if IsIPPort(input) {
		return workflowstruct.TypeIPPort
	} else if IsDomainPort(input) {
		return workflowstruct.TypeDomainPort
	} else if IsDomain(input) {
		return workflowstruct.TypeDomain
	} else if IsURL(input) {
		return workflowstruct.TypeURL
	}
...
func DeleteReportWithNoResult() {
	fileInfo, err := os.Stat(workflowstruct.GlobalConfig.ReportName)
	if err == nil {
		fileSize := fileInfo.Size()
		// 简单粗暴判断文件大小
		if fileSize < 99360 {
			_ = os.Remove(workflowstruct.GlobalConfig.ReportName)
		}
	}
}
