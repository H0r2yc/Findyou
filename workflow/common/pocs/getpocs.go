...
func GetPocs(workflowDB map[string]workflowstruct.WorkFlowEntity) (map[string][]string, int) {
	gologger.AuditTimeLogger("根据指纹选择Poc")
	result := make(map[string][]string)
	count := 0

	var generalKeys []string
	if !workflowstruct.GlobalConfig.DisableGeneralPoc {
	...
	for target, fingerprints := range workflowstruct.GlobalResultMap {
...
func addPocs(target string, result *map[string][]string, workflowEntity workflowstruct.WorkFlowEntity) {
	// 判断有没有加入过
	_, ok := (*result)[target]
	if !ok { // 没有添加过这个目标
