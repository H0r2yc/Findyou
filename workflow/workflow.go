...
func Workflowrun() {
	...
	for {
		gologger.Info().Msgf("当前workflow模块状态: \nFOFA搜索引擎:%v, HUNTER搜索引擎:%v, QUAKE搜索引擎:%v, CDN检查模块:%v, 子域名爆破模块:%v, 存活探测模块:%v, 目录扫描模块:%v, 主动指纹识别模块:%v, POC扫描模块:%v", appconfig.Module.Fofasearch, appconfig.Module.Huntersearch, appconfig.Module.Quakesearch, appconfig.Module.Cdncheck, appconfig.Module.SubDomainbrute, appconfig.Module.AliveAndPassivityScan, appconfig.Module.Dirbrute, appconfig.Module.Fingerprint, appconfig.Module.Pocscan)
		...
		case "DIRBRUTEANDPASSIVITYSCAN":
			dirbrute.DirBrute(value, appconfig)
		case "ACTIVEFINGERPRINT":
			activefingerprint.ActiveFingerprint(value, appconfig)
		default:
			if key == "" && value == nil {
		...
		//DONE 子域名爆破，超过一百个就立即删除否则会爆内存
		//TODO 重点IP做单独的端口扫描，1.真实的解析ip2.搜索结果较多的ip段3.targets中最多端口的域名或ipTOP?
		//Done Httpx做存活扫描
		//TODO 域名绑定资产发现
		//DONE 目录扫描，常见目录比如子域名同名目录，app，test,login等,进行指纹识别
