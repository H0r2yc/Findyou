...
func CheckPath(Path string,
	...
	inputChan := make(chan config.FingerPEntity, len(config.FingerprintDB))
	defer close(inputChan)
	results := make(chan string, len(config.FingerprintDB))
	defer close(results)
	...
	//添加扫描目标
	for _, input := range config.FingerprintDB {
		wg.Add(1)
		inputChan <- input
	}
	wg.Wait()
...
func FingerprintIdentification() {
	...
	// 先识别非Web
	for hostPort, protocol := range config.GlobalIPPortMap {
		...
		results := CheckPath("no#web", config.UrlPathEntity{}, port, protocol, banner, "")
		if len(results) > 0 {
			Url := fmt.Sprintf("%s://%s", protocol, hostPort)
			config.GlobalResultMap[Url] = results
...
func SingleCheck(finger config.FingerPEntity, Protocol string, headerString string, body string,
