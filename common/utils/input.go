...
func GetInputType(input string) int {
	if IsIPv6(input) {
		return config.TypeUnSupport
	} else if IsIPv4(input) {
		return config.TypeIP
	} else if IsIPRange(input) {
		return config.TypeIPRange
	} else if IsCIDR(input) {
		return config.TypeCIDR
	} else if IsIPPort(input) {
		return config.TypeIPPort
	} else if IsDomainPort(input) {
		return config.TypeDomainPort
	} else if IsDomain(input) {
		return config.TypeDomain
	} else if IsURL(input) {
		return config.TypeURL
	}
...
func DeleteReportWithNoResult() {
	fileInfo, err := os.Stat(config.GlobalConfig.ReportName)
	if err == nil {
		fileSize := fileInfo.Size()
		// 简单粗暴判断文件大小
		if fileSize < 99360 {
			_ = os.Remove(config.GlobalConfig.ReportName)
		}
	}
}
