...
func CheckCDNs(domains []string, threads int) (rCDNDomains []string, normalDomains []string, rIPs []string) {
	...
	defer close(Addrs)
	results := make(chan config.CDNResult, len(domains))
	defer close(results)
			...
			if result.IsCDN {
				...
				if len(result.IPs) != 0 {
					...
					for _, ip := range result.IPs {
						...
						config.GlobalIPDomainMapLock.Lock()
						_, ok := config.GlobalIPDomainMap[ip.String()]
						config.GlobalIPDomainMapLock.Unlock()
						if ok {
							// 存在于这个Map中
							config.GlobalIPDomainMapLock.Lock()
							dms, _ := config.GlobalIPDomainMap[ip.String()]
							config.GlobalIPDomainMapLock.Unlock()
							flag := false
							for _, dm := range dms {
								if dm == result.Domain {
							...
							if !flag { // 没有这个域名
								config.GlobalIPDomainMapLock.Lock()
								config.GlobalIPDomainMap[ip.String()] = append(config.GlobalIPDomainMap[ip.String()],
									result.Domain)
								config.GlobalIPDomainMapLock.Unlock()
							}
						} else {
							config.GlobalIPDomainMapLock.Lock()
							config.GlobalIPDomainMap[ip.String()] = []string{result.Domain}
							config.GlobalIPDomainMapLock.Unlock()
						}
					}
