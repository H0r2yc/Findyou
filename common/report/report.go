...
func GenerateHTMLReportHeader() {
	if config.GlobalConfig.ReportName == "" {
		config.GlobalConfig.ReportName = strconv.Itoa(int(time.Now().Unix())) + ".html"
	}
	showData := defaultHeader()
	writeFile(showData, config.GlobalConfig.ReportName)
}
...
<xmp>%s</xmp>
	...
	writeFile(d, config.GlobalConfig.ReportName)

	ReportIndex += 1
}

func AddResultByGoPocResult(result config.GoPocsResultType) {
